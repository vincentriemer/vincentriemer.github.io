...
// @codekit-prepend "AnimationFrame.js"
...
for ( var i = 0; i < paths.length; i++ ) {
	...
	var length = parseFloat(current_path.style.strokeDashoffset);
	current_path.style.strokeDasharray = length + ' ' + length;
	current_path.setAttribute("data-length", "" + length);
}

var rects = document.getElementsByTagName('rect');
for ( var i = 0; i < rects.length; i++ ) {
	var current_rect = rects[i];
	current_rect.setAttribute( "data-width", current_rect.getAttribute("width"));
	current_rect.setAttribute("width",0);
}

window.onload = function() {
	// Unhide the svg and start the animation once the page has finished loading
	document.getElementById("desktop-cover").style.display = "block";

	(function animate() {
		frameId = animationFrame.request(function() {
			...
			if ( progress > 1 ) {
				...
				current_frame++;
				for ( var i = 0; i < paths.length; i++ ) {
					var totalLength = parseFloat(paths[i].getAttribute("data-length"));
					paths[i].style.strokeDashoffset = Math.floor( totalLength * ( 1 - progress ) );
				}
				for ( var i = 0; i < rects.length; i++ ) {
					var fullWidth = parseFloat(rects[i].getAttribute("data-width"));
					rects[i].setAttribute( "width", Math.floor( fullWidth * ( progress ) ) );
				}
				animate();
			}
		})
